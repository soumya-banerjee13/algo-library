...
public class GraphForDijskstra {
  private Node[] nodes;
  private int noOfNodes;
  private Edge[] edges;
  private int noOfEdges;
  public GraphForDijskstra(Edge[] edges) {
    this.edges = edges;
    ...
    for (int i = 0; i < this.nodes.length; i++) {
      ...
      for (int joinedEdge = 0; joinedEdge < currentNodeEdges.size(); joinedEdge++) {
        ...
        if (!this.nodes[neighbourIndex].isVisited()) {
          long tentative = this.nodes[nextNode].getDistanceFromSource() + currentNodeEdges.get(joinedEdge).getWeight();
  ...
  private int getNodeShortestDistanced() {
    ...
    long storedDist = Long.MAX_VALUE;
    for (int i = 0; i < this.nodes.length; i++) {
      long currentDist = this.nodes[i].getDistanceFromSource();
